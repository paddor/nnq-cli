...
def ipc_url(label) = "ipc://@nnq-test-#{label}-#{SecureRandom.hex(4)}"


# Run PipeRunner in a dedicated thread so Ractor#join doesn't block the
# main thread's Async scheduler.
def run_pipe_runner(cfg)
  Thread.new do
    Sync do |task|
      NNQ::CLI::PipeRunner.new(cfg).call(task)
    end
  end
end
...
describe "pipe -P parallel execution" do
  it "routes all messages through workers and produces correct fib results" do
    ...
    cfg = make_config(
      type_name:     "pipe",
      in_endpoints:  [NNQ::CLI::Endpoint.new(work_url,    false)],
    ...
    io_thread = Thread.new do
      Sync do
        src  = NNQ::PUSH.new(linger: 1)
        src.bind(work_url)
        sink = NNQ::PULL.new(linger: 0)
        sink.bind(results_url)

        src.peer_connected.wait
        sink.peer_connected.wait
        (1..n_msgs).each { |n| src.send(n.to_s) }

        n_msgs.times.map { sink.receive.to_i }.sort
      ensure
        src&.close
        sink&.close
      end
  ...
  it "applies BEGIN/END blocks once per worker" do
    ...
    io_thread = Thread.new do
      Sync do
        ...
        2.times.map { sink.receive.to_i }.sum
      ensure
        src&.close
        sink&.close
      end
